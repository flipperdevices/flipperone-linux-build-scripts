...
| `build-flipctl.sh` | Builds flipctl (the panel UI) from `flipctl-slint` for arm64, natively or cross, and stages `/usr/bin/flipctl`, its remote-view assets, its unit and the sysusers, udev and modules-load files the unit needs for the ospack; no apps ship, they are installed from the catalogue; called by `build-ospack.sh` |
